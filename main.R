...
read_expression_table <- function(filename) {
  ...
  ex_int_data <- read_delim(filename, delim = " ", col_names = FALSE) %>%
    as_data_frame() %>%
    t() %>%
    as_tibble()
  
  #rename columns with first row of data
  colnames(ex_int_data) <- ex_int_data[1,]
  
  #remove first row and rename "probe" column to "subject_id"
  ex_int_data <- ex_int_data[-1,] %>%
    dplyr::rename("subject_id" = "probe")
...
period_to_underscore <- function(str) {
  return (str_replace_all(str, fixed("."), "_"))
}
